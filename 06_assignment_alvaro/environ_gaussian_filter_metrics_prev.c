...
// void processImage(GPUContext& gpu_ctx, const std::vector<unsigned char> rgba_in, int width, int height
//     float sigma, int maskSize ,float * mask, int side ,cl_image_desc img_desc,cl_image_format img_format, std::atomic<int>& remaining_threads) {
//     ImageMetrics metrics;
//     metrics.gpu_id = gpu_ctx.gpu_id;
//     // metrics.filename = fs::path(image_path).filename().string();
//     metrics.filename = "cat_1000x600.jpg";
...
int main(int argc, const char *argv[]) {
    ...
    // =====================================================
    // 5. Execute FULL PIPELINE 5000 times
    // =====================================================
    const int N = 5000;
    size_t global_size[2] = {(size_t)width, (size_t)height};
    ...
    for (int i = 0; i < N; ++i) {
        GPUContext& ctx = (i % 2 == 0) ? gpu0_ctx : gpu1_ctx;
        cl_mem& img_in  = (i % 2 == 0) ? img0_in  : img1_in;
        cl_mem& img_out = (i % 2 == 0) ? img0_out : img1_out;
        ...
        clEnqueueReadImage(ctx.command_queue,
                           ...
                           origin, region,
                           0, 0, tmp.data(),
                           0, &evt_kernel, &evt_d2h);

        clWaitForEvents(1, &evt_d2h);

        h2d_ms    += eventDurationMs(evt_h2d);
        kernel_ms += eventDurationMs(evt_kernel);
        d2h_ms    += eventDurationMs(evt_d2h);

        clReleaseEvent(evt_h2d);
        clReleaseEvent(evt_kernel);
        clReleaseEvent(evt_d2h);
    }
    ...
    // =====================================================
    // 6. Print meaningful metrics
    // =====================================================
    std::cout << "\n==============================\n";
    std::cout << "Total iterations:     " << N << "\n";
    std::cout << "Total time:           " << total_ms << " ms\n";
    std::cout << "Avg per iteration:    " << total_ms / N << " ms\n";
    std::cout << "Avg per GPU:          " << total_ms / (N/2) << " ms\n";
    std::cout << "==============================\n";

    double bytes_per_image = width * height * 4;
    double total_bytes = bytes_per_image * N;

    double h2d_bw = (total_bytes / (1024.0 * 1024.0)) / (h2d_ms / 1000.0);
    double d2h_bw = (total_bytes / (1024.0 * 1024.0)) / (d2h_ms / 1000.0);

    std::cout << "\n================ GPU PIPELINE BENCHMARK ================\n";
    std::cout << "Iterations:            " << N << "\n";
    std::cout << "Image size:            " << width << " x " << height << "\n\n";

    std::cout << "Total H2D time:        " << h2d_ms << " ms\n";
    std::cout << "Total Kernel time:     " << kernel_ms << " ms\n";
    std::cout << "Total D2H time:        " << d2h_ms << " ms\n\n";

    std::cout << "Avg H2D per iter:      " << h2d_ms / N << " ms\n";
    std::cout << "Avg Kernel per iter:   " << kernel_ms / N << " ms\n";
    std::cout << "Avg D2H per iter:      " << d2h_ms / N << " ms\n\n";

    std::cout << "H2D Bandwidth:         " << h2d_bw << " MB/s\n";
    std::cout << "D2H Bandwidth:         " << d2h_bw << " MB/s\n";

    // =====================================================
    // 7. Read FINAL result and save
    // =====================================================
    std::vector<unsigned char> outRGBA(width * height * 4);
    clEnqueueReadImage(gpu0_ctx.command_queue,
                       img0_out, CL_TRUE,
                       origin, region,
                       0, 0, outRGBA.data(),
                       0, nullptr, nullptr);
...
//     std::cout << "Starting concurrent processing with 50/50 load balancing...\n" << std::endl;
...
//     const int N = 5000;
//     std::atomic<int> remaining_threads{N}; // To save output of last image
...
//         threads.emplace_back(processImage, std::ref(gpu_ctx), rgba_in, width, height, sigma, maskSize, mask, side, img_desc, img_format);
